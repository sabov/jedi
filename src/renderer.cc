...
//
void APIENTRY ogl_debug_callback(GLenum _source, GLenum _type, GLuint _id, GLenum _severity, GLsizei /*_length*/,
        const GLchar *_message, void* /* _userParam*/) {
    if (_type == GL_DEBUG_TYPE_ERROR) {
        error() << "<" << _id << "> severity: " << debugSeverityName(_severity) << " source: " << debugSourceName(_source)
                << ": " << _message << endl;
        // Set a breakpoint in the line above! This way you get a stack trace in case something fails!
    } else {
        debug() << "<" << _id << "> severity: " << debugSeverityName(_severity) << " source: " << debugSourceName(_source)
                << ": " << _message << endl;
    }
